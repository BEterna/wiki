...
To be able to preview and export personal tax report, it is necessary to select electronic reporting formats for PDD-GI. 
...
It is necessary to set up income types for reporting purposes. Using button “New” entry with income type and description is created. Income type is constructed in format: “income type.subincome.paymenttype”. 
...
Create non-cash types for reporting non-cash personal receivables. Define default sales tax group and sales tax item group, select offset account, and define whether tax is included for each of the types.  
...
New sales tax settlement period should be created for personal tax purposes. 
...
New ledger posting group is opened for ledger posting. According to our business practice, we open as much ledger posting groups as there are different ledger accounts we want to post liabilities to. 
...
Personal tax income type is defined on sales tax code. For every income type there are sales tax codes with personal tax setup. 

Under General, settlement period and ledger posting group, created for personal tax purposes, and currency are defined.  
...
Add adequate sales tax codes to sales tax groups. 
...
Item sales tax group (from procurement category) and sales tax group (from vendor) are automatically populated in line details – Setup.  
...
Here, non-cash personal receivables can be defined. Select Yes for line to be marked as non-cash receivable, define non-cash type, and select the receiving worker.  

 

After the invoice is posted, non-cash receivables can be generated in Accounts payable – Non cash personal receivables – Non cash personal receivables. 

Use button New to create new document. 

 

Add date and select Non-cash type. Upon selecting non-cash type, sales tax group, item sales tax group, and offset account, defined on the type, are transferred to the document.  

 

Click “Import vendor invoice lines” to transfer invoice lines. Define dates From and To (period for posted vendor transaction).  

 

Upon confirming, non-cash personal receivable line is automatically (after Refresh) created with reference to vendor invoice.  

 

 Post non-cash receivables with button Post. 

 

See chapter Other transactions on how to create personal tax payments for non-cash receivables. 
...
Using this functionality, sales tax report is created, enabling user to check tax transactions for personal tax before posting them.  

Select settlement period for personal tax and define “From date” (any date in a month that needs to be settled). 

 

After confirming, another window is opened. Enable “Include details” if additional section of the report “Details” is to be displayed in sums for each sales tax code.  

 

Confirming generates sales tax report. 

 

 

Tax – Declarations – Settle and post sales tax 
